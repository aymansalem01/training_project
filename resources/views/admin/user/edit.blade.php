...
<div class="form-container">
    ...
    <form action="{{ route('user.update', $user->id) }}" method="POST">
        ...
        <div>
            ...
            <input type="password" name="password" id="password" class="form-control mb-3" placeholder="Enter a new password">
            @error('password')
                <div class="custom-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="text" name="phone_number" id="phone_number" class="form-control mb-3" pattern="^07[0-9]{8}$"
                value="{{ old('phone_number', $user->phone_number) }}" placeholder="07XXXXXXXX" required>
            @error('phone_number')
                <div class="custom-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>
            @enderror
        </div>
